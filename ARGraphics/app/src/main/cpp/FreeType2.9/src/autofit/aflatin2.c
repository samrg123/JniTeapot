    ...
    for ( bb = 0; bb < AF_LATIN_BLUE_MAX; bb++ )
    {
      ...
      for ( ; p < limit && *p; p++ )
      {
        ...
        /* lies, then inspect its previous and next points              */
        {
          ...
          FT_Int  start, end, previousBlock, next;
          FT_Pos  dist;
          ...
          do
          {
            previousBlock = start - 1;
            if ( previousBlock < best_first )
              previousBlock = best_last;

            dist = FT_ABS( points[previousBlock].y - best_y );
            /* accept a small distance or a small angle (both values are */
            /* heuristic; value 20 corresponds to approx. 2.9 degrees)   */
            if ( dist > 5 )
              if ( FT_ABS( points[previousBlock].x - best_x ) <= 20 * dist )
                break;
...
#define  SORT_SEGMENTS
                                    ...
                                    AF_Dimension   dim )
  {
    ...
    for ( ; contour < contour_limit; contour++ )
    {
      ...
      AF_Point  last    =  point->previousBlock;
