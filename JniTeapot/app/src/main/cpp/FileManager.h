...
class FileManager {
    ...
    public:
        ...
        static void Init(JNIEnv* env, jobject jAssetManager) {
            ...
            RUNTIME_ASSERT(assetManager, "Failed to Initialize { requested jAssetManger: %p }", jAssetManager);
            
            Log("Initialed assetManger { jAssetManger: %p, assetManager: %p }", jAssetManager, assetManager);
        }
        ...
        static void SaveAsFile(AssetBuffer* buffer, const char* filePath) {
            ...
            RUNTIME_ASSERT(fd >= 0, "Failed to open file for writing { assetBuffer: %p, filePath: %s, linux errono: %d }", buffer, filePath, errno);
            
            size_t bytesWritten = write(fd, buffer->data, buffer->size);
            RUNTIME_ASSERT(bytesWritten >= 0, "Failed to write buffer to file { assetBuffer: %p, filePath: %s, linux errono: %d}", buffer, filePath, errno);
