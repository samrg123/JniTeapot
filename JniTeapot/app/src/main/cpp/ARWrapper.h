...
class ARWrapper {
    ...
    public:
        ...
        inline glm::mat4 ProjectionMatrix_glm(float nearPlane, float farPlane) const {
            RUNTIME_ASSERT(arSession, "arSession not Initialized");

            glm::mat4 projMatrix;
            ArCamera_getProjectionMatrix(arSession, arCamera, nearPlane, farPlane, glm::value_ptr(projMatrix));
            return projMatrix;
        }
