...
//        Log("Could not load png texture for planes.");
//    }
//
...
//
    GlAssertNoError("PlaneRenderer::InitializeGlContent() error");
}
...
void PlaneRenderer::Draw(const glm::mat4& projection_mat,
    ...
    if (!shader_program_) {
        Log("shader_program is null.");
        return;
    }

    vertices_.clear();
    vertices_.push_back(glm::vec3(-100, -100, 0));
    vertices_.push_back(glm::vec3(-100,  100, 0));
    vertices_.push_back(glm::vec3( 100, -100, 0));
...
//    glBindTexture(GL_TEXTURE_2D, texture_id_);
    ...
    glUseProgram(0);
    glDepthMask(GL_TRUE);
    GlAssertNoError("PlaneRenderer::Draw() error");
}
...
void PlaneRenderer::UpdateForPlane(const ArSession& ar_session,
    ...
    if (polygon_length == 0) {
        Log("PlaneRenderer::UpdatePlane, no valid plane polygon is found");
        return;
    }
